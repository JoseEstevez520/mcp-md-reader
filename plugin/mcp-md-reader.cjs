...
async function setCached(path, parsed) {
  try {
    const s = await (0, import_promises.stat)(path);
    evictIfNeeded();
    const entry = {
...
var CACHE_DIR = (0, import_node_path.join)((0, import_node_os.tmpdir)(), "mcp-md-reader-cache");
var CACHE_INDEX_PATH = (0, import_node_path.join)(CACHE_DIR, "index.json");
var diskIndexLoaded = false;
var diskIndex = { version: 1, entries: {} };
...
async function loadDiskIndex() {
  ...
  try {
    ...
    const raw = await (0, import_promises.readFile)(CACHE_INDEX_PATH, "utf-8");
...
async function saveDiskIndex() {
  try {
    await ensureCacheDir();
    const keys = Object.keys(diskIndex.entries);
    if (keys.length > MAX_ENTRIES) {
      const sorted = keys.sort(
        (a, b) => diskIndex.entries[a].lastAccess - diskIndex.entries[b].lastAccess
      );
      for (let i = 0; i < sorted.length - MAX_ENTRIES; i++) {
        delete diskIndex.entries[sorted[i]];
      }
    }
    await (0, import_promises.writeFile)(CACHE_INDEX_PATH, JSON.stringify(diskIndex), "utf-8");
  } catch {
  }
}
async function getDiskCache(path) {
  await loadDiskIndex();
...
async function setDiskCache(path, entry) {
  await loadDiskIndex();
  ...
  for (const line of fmLines) {
    ...
    if (match) {
      ...
      if (value === "") {
        ...
        parsed[currentKey] = value.slice(1, -1).split(",").map((s) => s.trim()).filter((s) => s.length > 0);
        currentKey = null;
      } else {
...
function findHeadings(lines, startLine) {
  const headings = [];
...
function sliceContent(lines, from, to) {
  return lines.slice(from, to + 1).join("\n");
}
function buildTree(headings, lines) {
  if (headings.length === 0) return [];
  const nodes = headings.map((h, i) => {
    const lineEnd = i < headings.length - 1 ? headings[i + 1].lineIndex - 1 : lines.length - 1;
...
function matchesWordBoundary(q, target) {
  const words = target.split(/[\s_\-/]+|(?<=[a-z])(?=[A-Z])/);
  for (const w of words) {
    if (w.toLowerCase().startsWith(q)) return true;
  }
  if (q.length >= 2 && q.length <= 5) {
    const initials = words.filter((w) => w.length > 0).map((w) => w[0].toLowerCase()).join("");
...
function fuzzyScore(query, target) {
  ...
  if (q.length >= 2 && q.length < 4) {
    ...
    const tWords2 = t.split(/\s+/);
    if (tWords2.some((w) => w === q)) return 0.9;
    return 0;
  }
...
async function parseMarkdownCached(path, text) {
...
var INDEX_DIRNAME = ".mcp-md-reader";
var INDEX_FILENAME = "graph-index.json";
var STALE_MS = 60 * 60 * 1e3;
var IGNORE_DIRS = /* @__PURE__ */ new Set(["node_modules", ".git", ".obsidian", "ATTACHMENTS", ".mcp-md-reader"]);
...
function extractFrontmatter2(raw) {
...
function extractStructure(strippedContent, lineOffset) {
...
function buildHeadingTree(flat) {
  ...
  const stack = [];
  for (const h of flat) {
    const node = { title: h.title, level: h.level, line: h.line };
...
function extractLinks(strippedContent) {
...
async function compileIndex(vaultRoot) {
  const files = await walkMd(vaultRoot);
  const nameCount = /* @__PURE__ */ new Map();
  for (const f of files) {
    const name = normalizeId((0, import_node_path2.basename)(f));
    nameCount.set(name, (nameCount.get(name) || 0) + 1);
  }
  const nodes = {};
  for (const filePath of files) {
    const raw = await (0, import_promises2.readFile)(filePath, "utf-8");
    const relPath = (0, import_node_path2.relative)(vaultRoot, filePath).replace(/\\/g, "/");
    const baseName = normalizeId((0, import_node_path2.basename)(filePath));
    const id = (nameCount.get(baseName) || 0) > 1 ? normalizeId(relPath.replace(/\//g, "_")) : baseName;
    const [frontmatter, bodyStartLine] = extractFrontmatter2(raw);
  ...
  const simpleNameToId = /* @__PURE__ */ new Map();
  for (const [id, node] of Object.entries(nodes)) {
    const simpleName = normalizeId((0, import_node_path2.basename)(node.path));
    if (simpleNameToId.has(simpleName)) {
      simpleNameToId.set(simpleName, null);
    } else {
...
function indexPath(vaultRoot) {
  const dir = process.env.MD_READER_INDEX_DIR || (0, import_node_path2.join)(vaultRoot, INDEX_DIRNAME);
  return (0, import_node_path2.join)(dir, INDEX_FILENAME);
}
async function getIndex(vaultRoot) {
  ...
  try {
    const s = await (0, import_promises2.stat)(idxPath);
  ...
  if (needsCompile) {
    ...
    const dir = (0, import_node_path2.dirname)(idxPath);
    await (0, import_promises2.mkdir)(dir, { recursive: true });
    await (0, import_promises2.writeFile)(idxPath, JSON.stringify(index, null, 2), "utf-8");
    cachedIndex = index;
    cachedVault = vaultRoot;
    cachedAt = Date.now();
    return index;
  }
  const raw = await (0, import_promises2.readFile)(idxPath, "utf-8");
  cachedIndex = JSON.parse(raw);
  cachedVault = vaultRoot;
  cachedAt = Date.now();
  return cachedIndex;
}
async function queryIndex(vaultRoot, query, nodeId, depth) {
...
function formatStats(meta, nodes) {
  const typeCounts = /* @__PURE__ */ new Map();
  for (const node of Object.values(nodes)) {
    const type = node.frontmatter?.type ?? node.frontmatter?.tipo ?? "untyped";
    typeCounts.set(type, (typeCounts.get(type) || 0) + 1);
  }
  const typeLines = [...typeCounts.entries()].sort((a, b) => b[1] - a[1]).map(([t, c]) => `  ${t}: ${c}`).join("\n");
  return [
    `Vault: ${meta.vault}`,
...
function formatNode(nodes, nodeId) {
...
function formatNeighbors(nodes, nodeId, depth = 1) {
...
function formatSearchType(nodes, typeQuery) {
  if (!typeQuery) return 'Error: node_id (= type to search) is required for query "search_type"';
  const matches = [];
  for (const [id, node] of Object.entries(nodes)) {
    const type = node.frontmatter?.type ?? node.frontmatter?.tipo ?? "";
...
function formatMostConnected(nodes, n) {
  const ranked = Object.entries(nodes).map(([id, node]) => ({
    id,
...
function formatIsolated(nodes) {
  const isolated = Object.entries(nodes).filter(([, node]) => node.links_out.length === 0 && node.links_in.length === 0).map(([id, node]) => `  ${id} \u2014 ${node.path}`);
  if (isolated.length === 0) return "No isolated nodes found.";
  return `Isolated nodes (${isolated.length}):
${isolated.join("\n")}`;
}
function formatPath(nodes, nodeId) {
...
var FIND_BUDGET_TOKENS = 4e3;
var MAX_REGIONS = 12;
var AMBIGUOUS_DOC_SPREAD = 20;
...
function tokenize(s) {
  return s.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter((w) => w.length >= 3 && !STOPWORDS.has(w));
}
...
async function findInVault(vaultRoot, query) {
  ...
  for (const [id, node] of Object.entries(nodes)) {
    ...
    }).filter((h) => h.hits > 0).sort((a, b) => b.hits - a.hits);
    if (covered.size === 0) continue;
    const shownHeadings = headings.length > 0 ? headings : flat.slice(0, 4).map((h) => ({ title: h.title, breadcrumb: h.breadcrumb, hits: 0 }));
    matches.push({
      path: node.path,
      absPath: (0, import_node_path2.join)(vaultRoot, node.path),
  ...
  if (matches.length === 0) {
    const hubs = Object.values(nodes).map((n) => ({ path: n.path, deg: n.links_in.length + n.links_out.length })).sort((a, b) => b.deg - a.deg).slice(0, 8).map((h) => `  ${h.path}`);
    return [
  ...
  matches.sort(
    (a, b) => b.coverage - a.coverage || b.headings[0].hits - a.headings[0].hits || b.linksIn - a.linksIn
  );
  if (matches.length > AMBIGUOUS_DOC_SPREAD) {
    const top = matches.slice(0, 15).map(
      (m) => `  ${m.path.padEnd(50)} (${m.headings.length} sec, in:${m.linksIn})`
    );
    return [
...
var VALID_QUERIES = ["stats", "node", "neighbors", "search_type", "most_connected", "isolated", "path"];
var tools = [
  {
    name: "md_find",
...
function savings(part, whole) {
  return whole === 0 ? 0 : Math.round((1 - part / whole) * 100);
}
function categorizeError(error, path) {
  const e = error;
...
async function loadFile(path) {
  let metadata;
  try {
    metadata = await (0, import_promises3.stat)(path);
  } catch (error) {
  ...
  const buffer = await (0, import_promises3.readFile)(path);
...
async function callTool(name, args) {
  try {
    ...
    if (name === "md_vault_index") {
      ...
      if (!VALID_QUERIES.includes(query)) throw new Error(`Unsupported query: ${query}`);
      const nodeId = args.node_id === void 0 ? void 0 : requireString(args, "node_id");
      if (args.depth !== void 0 && typeof args.depth !== "number") throw new Error("depth must be a number");
      return textResult(await queryIndex(vaultPath, query, nodeId, args.depth));
    }
...
async function handle(request) {
  ...
  try {
    ...
    if (request.method === "tools/call") {
      ...
      write({ ...base, result: await callTool(name, rawArguments ?? {}) });
      return;
    }
...
var input = (0, import_node_readline.createInterface)({ input: process.stdin, terminal: false });
input.on("line", (line) => {
